  ...
  return (
    <div className="p-8">
      ...
      <div className="grid lg:grid-cols-2 gap-8 mb-8">
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between gap-4">
            <h2 className="text-sm font-semibold text-gray-700">Classroom Breakdown</h2>
            <input
              type="search"
              placeholder="Filter classrooms..."
              value={classroomFilter}
              onChange={(e) => setClassroomFilter(e.target.value)}
              className="text-xs px-3 py-1.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-40"
              aria-label="Filter classrooms"
            />
          </div>
          <div className="p-6 space-y-5">
            {filteredClassrooms.length === 0 ? (
              <p className="text-sm text-gray-400 italic">No classrooms match your filter.</p>
            ) : (
              filteredClassrooms.map((cls) => (
                <div key={cls.classroomId}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-gray-700">{cls.classroomName}</span>
                    <span className="text-sm font-bold text-gray-900">{cls.averageCompletion}%</span>
                  </div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">{cls.studentCount} students</p>
                </div>
              ))
            )}
          </div>
            ...
            {analytics.subjectBreakdown.map((sub) => (
              <div key={sub.subjectId}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-bold text-gray-900">{sub.percentage}%</span>
                </div>
                <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
                  <div
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
        <h2 className="text-sm font-semibold text-gray-700 mb-4">School Summary</h2>
        <div className="grid sm:grid-cols-3 gap-6">
          ...
          <div className="text-center">
            <p className="text-3xl font-bold text-gray-900">{analytics.averageCompletion}%</p>
