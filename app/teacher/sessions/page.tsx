...
const modeBadge: Record<string, string> = {
  teach: 'bg-red-900/20 text-red-700 border-red-200',
  ...
  return (
    <div className="p-8">
      <div className="mb-8 flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Classroom Sessions</h1>
      ...
      {allMySessions.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h2 className="text-gray-700 font-semibold text-lg mb-2">No sessions yet</h2>
        ...
        <>
          <div className="grid grid-cols-4 gap-4 mb-8">
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <p className="text-xs text-gray-500 uppercase tracking-wide font-semibold mb-1">Total Sessions</p>
              <p className="text-3xl font-bold text-gray-900">{allMySessions.length}</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <p className="text-xs text-gray-500 uppercase tracking-wide font-semibold mb-1">Teach Mode</p>
              <p className="text-3xl font-bold text-red-600">
                {allMySessions.filter((s) => s.mode === 'teach').length}
              </p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <p className="text-xs text-gray-500 uppercase tracking-wide font-semibold mb-1">Quiz Mode</p>
              <p className="text-3xl font-bold text-blue-600">
                {allMySessions.filter((s) => s.mode === 'quiz').length}
              </p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <p className="text-xs text-gray-500 uppercase tracking-wide font-semibold mb-1">Avg Quiz Score</p>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="text-xs px-3 py-1.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                <div className="flex items-center gap-2">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-6 py-3">Topic</th>
                    <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-6 py-3">Subject</th>
                    <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-6 py-3">Classroom</th>
                    <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-6 py-3">Mode</th>
                    <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-6 py-3">Score</th>
                    <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-6 py-3">Duration</th>
                  ...
                  {mySessions.map((session: LessonSession) => (
                    <tr key={session.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4">
                        <span className="font-medium text-gray-900 text-sm">{session.topicName}</span>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-600">{session.subjectName}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{session.classroomName}</td>
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border capitalize ${
                            modeBadge[session.mode] ?? 'bg-gray-100 text-gray-600 border-gray-200'
                          }`}
                        >
                          {session.mode}
                        </span>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-500">{formatDate(session.startedAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
