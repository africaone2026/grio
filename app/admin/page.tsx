...
export default function AdminAnalyticsPage() {
  ...
  const roleBreakdown = [
    ...
    { label: 'Super Admins', count: users.filter((u) => u.role === 'super_admin').length, color: 'bg-gray-500' },
  ];
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm mt-1">Live overview of GRIO platform usage.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h2 className="font-bold text-gray-900 mb-5">User Breakdown by Role</h2>
          <div className="space-y-4">
            {roleBreakdown.map((role) => (
              <div key={role.label} className="flex items-center gap-4">
                <span className="w-28 text-sm text-gray-600">{role.label}</span>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h2 className="font-bold text-gray-900 mb-5">Subscription Status</h2>
          <div className="space-y-4">
            <ProgressBar
          ...
          <div className="mt-6 pt-6 border-t border-gray-100">
            <p className="text-sm text-gray-500">
              ...
              <span className="font-bold text-gray-900">{analytics.averageCompletion}%</span>
            </p>
          </div>
        </div>
      </div>
      ...
      {global && global.countryBreakdown.length > 0 && (
        <div className="mt-8 bg-white rounded-xl border border-gray-200 p-6">
          <h2 className="font-bold text-gray-900 mb-5">Country Breakdown</h2>
          <div className="grid sm:grid-cols-3 gap-6">
            {global.countryBreakdown.map((c) => (
              <div key={c.countryId} className="border border-gray-100 rounded-xl p-5">
                <p className="text-sm font-semibold text-gray-800 mb-1">{c.countryName}</p>
                ...
                <p className="text-xs text-gray-500 mt-1">{c.averageCompletion}% avg completion</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-xl font-bold text-gray-900">Classroom Sessions</h2>
          {sessions.length > 0 && (
        ...
        {sessions.length > 0 ? (
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h3 className="font-semibold text-gray-900 mb-4">Sessions by School</h3>
              {Object.keys(sessionsBySchool).length === 0 ? (
                <p className="text-sm text-gray-400">No school data available.</p>
              ) : (
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h3 className="font-semibold text-gray-900 mb-4">Sessions by Subject</h3>
                ...
                <div className="space-y-3">
                  {Object.entries(sessionsBySubject).map(([subject, count]) => (
                    <div key={subject} className="flex items-center gap-4">
                      <span className="w-28 text-sm text-gray-600 truncate">{subject}</span>
                      ...
                      <span className="w-6 text-sm font-semibold text-gray-700 text-right">{count}</span>
                    </div>
                  ))}
                </div>
              )}
